...
/**
 * Test cases for reading file content for systems where each FileObject is FileType.FILE_OR_FOLDER.
 */
public class FileOrFolderContentTests extends AbstractProviderTestCase {
  ...
  private void assertSameContent(final FileInfo expected, final FileObject folder) throws Exception {
    for (final FileInfo fileInfo : expected.getChildren().values()) {
      final FileObject child = folder.resolveFile(fileInfo.getBaseName(), NameScope.CHILD);

      assertTrue(child.getName().toString(), child.exists());
      if (fileInfo.getType() == FileType.FILE) {
        assertSameContent(fileInfo.getContent(), child);
      } else {
  ...
  public void testChildren() throws FileSystemException {
    ...
    } else if (FileType.FILE_OR_FOLDER == file.getType()) {
      // we SHOULD be able to get children
    ...
    // this file is VIRTUAL UNTIL IT EXISTS
    assertTrue(FileType.IMAGINARY == file.getType());
    // in a FileOrFolder system, any file may or may not have children
    // so it will not throw if there are no children
    // even if IMAGINARY
  ...
  public void testReuse() throws Exception {
    // Get the test file
    final FileObject file = getReadFolder().resolveFile("file1.txt");
    assertEquals(FileType.FILE_OR_FOLDER, file.getType());
