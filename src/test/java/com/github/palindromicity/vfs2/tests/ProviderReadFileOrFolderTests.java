 ...
 */
public class ProviderReadFileOrFolderTests extends AbstractProviderTestCase {
  ...
  protected org.apache.commons.vfs2.test.FileInfo buildExpectedStructure() throws FileSystemException {
    ...
    base.addFileOrFolder("file space.txt", FILE1_CONTENT);

    base.addFileOrFolder("empty.txt", "");
    if (getAddEmptyDir()) {
  ...
  protected void assertSameStructure(final FileObject folder, final FileInfo expected) throws Exception {
    ...
    while (queueActual.size() > 0) {
      ...
      assertSame(FileType.FILE_OR_FOLDER, file.getType());

      // check the original to see if there should be children
      ...
      if (info.getChildren().size() != children.length) {
      ...
      assertEquals("count children of \"" + file.getName() + "\"", info.getChildren().size(), length);
      ...
      for (final FileObject child : children) {
        ...
        final FileInfo childInfo = info.getChildren().get(childName);
  ...
  /**
   * Tests that folders may have content.
   */
  public void testFolderContent() throws Exception {
    ...
    try {
      folder.getContent().getInputStream().close();
